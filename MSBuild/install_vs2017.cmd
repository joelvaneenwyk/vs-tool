...
rem On 64-bit machines, Visual Studio 2015 and MsBuild is in the (x86) directory. So try that last.

if exist "%ProgramFiles%" set MsBuildCppDir=%ProgramFiles%\MSBuild\Microsoft.Cpp\v4.0\V140\Platforms
if exist "%ProgramFiles(x86)%" set MsBuildCppDir=%ProgramFiles(x86)%\MSBuild\Microsoft.Cpp\v4.0\V140\Platforms
...
IF ERRORLEVEL 1 (
	...
	if not exist "%MsBuildCppDir%" (
		...
		Echo.Failed to find the MsBuild directories that should have been installed with 'Visual Studio 2015'.
		Echo."%MsBuildCppDir%"
		Pause
		goto cleanup
	...
	xcopy "2017 DLL\*.dll" "%MsBuildCppDir%\Android" /E
